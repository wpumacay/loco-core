...
class LOCO_API SingleBodyCollider {
 ...
 public:
    /// Creates a single body collider using the given configuration
    ...
    /// Changes the size of the collider
    /// \param[in] size The new size of the collider
    auto ChangeSize(Vec3 size) -> void;

    /// Changes the vertex data of the mesh collider (if applicable)
    ...
    /// Changes the elevation data of the hfield collider (if applicable)
    ...
    LOCO_NODISCARD auto shape() const -> eShapeType { return m_Data.type; }

    /// Returns the size of the collider
    LOCO_NODISCARD auto size() const -> Vec3 { return m_Data.size; }

    /// Returns the collision group of the collider
    LOCO_NODISCARD auto collision_group() const -> int32_t {
        return m_Data.collision_group;
    }

    /// Returns the collision mask of the collider
    LOCO_NODISCARD auto collision_mask() const -> int32_t {
        return m_Data.collision_mask;
    }

    /// Returns an unmutable reference to the config data of the collider
    LOCO_NODISCARD auto data() const -> const ColliderData& { return m_Data; }
