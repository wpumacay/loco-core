...
class LOCO_API Scenario : public std::enable_shared_from_this<Scenario> {
 ...
 public:
    ...
    /// Adds a given free drawable to the scenario with the given name
    ...
    /// Returns the drawable at given index
    /// \param[in] index The index of the drawable we want to retrieve
    LOCO_NODISCARD auto GetDrawableByIndex(size_t index) -> Drawable::ptr;

    /// Returns the drawable with given name
    /// \param[in] name The name of the drawable we want to retrieve
    LOCO_NODISCARD auto GetDrawableByName(const std::string& name)
        -> Drawable::ptr;

    /// Returns the current number of free drawables in this scenario
    LOCO_NODISCARD auto num_drawables() const -> size_t;
