...
def test_core_components_data() :
    col = sim.CollisionData()
    col.type = sim.ShapeType.SPHERE
    ...
    assert col.type == sim.ShapeType.SPHERE, "test_core_components_data >>> couldn't set collider-param 'type'"
    ...
    assert vis.type == sim.ShapeType.SPHERE, "test_core_components_data >>> couldn't set visual-param 'type'"
    ...
    jnt = sim.JointData()
    jnt.type = sim.JointType.REVOLUTE
    jnt.axis = [ 1., 0., 0. ]
    ...
    assert jnt.type == sim.JointType.REVOLUTE, "test_core_components_data >>> couldn't set joint-param 'type'"
    ...
    act = sim.ActuatorData()
    act.type = sim.ActuatorType.PD_CONTROLLER
    ...
    assert act.type == sim.ActuatorType.PD_CONTROLLER, "test_core_components_data >>> couldn't set actuator-param 'type'"
    ...
    sen = sim.SensorData()
    sen.type = sim.SensorType.EXT_CAMERA_RGB
    sen.localTf = tm.Matrix4f( np.identity( 3 ), [ 0.0, 0.1, 0.2 ] )
    assert sen.type == sim.SensorType.EXT_CAMERA_RGB, "test_core_components_data >>> couldn't set sensor-param 'type'"
    ...
    body = sim.BodyData()
    body.dyntype = sim.DynamicsType.DYNAMIC
    body.inertialData = inertia
    body.localTf0 = tm.Matrix4f( np.identity( 3 ), [ -0.01, 0.02, -0.03 ] )
    body.collision = col
    body.visual = vis
    assert body.dyntype == sim.DynamicsType.DYNAMIC, "test_core_components_data >>> couldn't set body-param 'dyntype'"
